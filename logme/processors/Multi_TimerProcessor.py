...
class Multi_TimerProcessor:
    ...
    def landing_to_raw(self) -> list[pd.DataFrame]:
        ...
        if self.ProcessingUtils._table_exists(f'{self.src}_raw') != True:
            ...
            query = self.ProcessingUtils._query_from_list_of_fields(self.src, "raw", self.conf["fields"], self.conf["fields_format"])
            if self.ProcessingUtils._create_table(query) != SUCCESS:
                raise typer.Exit(f"Error creating {self.src}_raw")
        dfs = []
        for file in self.files:
            dfs.append(self.ProcessingUtils._ingest_file_to_db(file, f'{self.src}_raw', self.conf))
        return dfs

    def check_data_quality(self) -> int:
        # If conf has header, check the names
        if self.conf['header']:
            if self.ProcessingUtils._are_headers_correct(self.files, self.conf):
    ...
    def raw_to_l1(self, dfs: list[pd.DataFrame]) -> list[pd.DataFrame]:
        dfs_casted = []
        if self.ProcessingUtils._table_exists(f'{self.src}_l1') != True:
        ...
        for df in dfs:
            print(df.info())
            # print(df[cols_raw[5]])
            df_casted = self.ProcessingUtils._raw_to_l1_types(df,self.conf, self.conf_raw_to_l1)
    ...
    def l1_to_l2(self, dfs: list[pd.DataFrame]) -> list[pd.DataFrame]:
        dfs_out = []
        if self.ProcessingUtils._table_exists(f'{self.src}_l2') != True:
