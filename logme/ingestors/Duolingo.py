...
class DuolingoApi:
    ...
    def download_end_points(self) -> None:
        ...
        cal = str(lingo.get_calendar()).replace("None", "'None'").replace("'", '"')
    ...
    def process(self, learned_skills: list) -> pd.DataFrame:
        ...
        for skill in learned_skills:
            try:
                ...
                learned_skills_short.append(skill["short"])
                # logger.info(skill['language'])
                learned_skills_language.append(languages_dict[skill["language"]])
            except KeyError:
