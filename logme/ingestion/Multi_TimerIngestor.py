...
#from logme.connectors.Dropbox import DropboxConnector
import logme.connectors.Dropbox as dropbox
import typer
...
import time
import numpy as np
from logme.utils.Utils import move_file_in_local_system
import logme.utils.Utils

class MultiTimerIngest:
    """
    Class to ingest Multi Timer app data
    """
    ...
    def __init__(self, src: Path, dst: Path, conf: dict) -> None:
        ...
        src_history_path = Path(f'{history_path}/{src}')
        if not path.exists(src_history_path):
            self.logger.info(f'Creating history path for {self.src}: {src_history_path}')
            makedirs(src_history_path)

    def ingest_to_landing(self) -> list:
        """
        Moves the files from the _connection_ in config.ini to logme_data local
        :return:
        """
        if self.conf['connection'] == 'Dropbox':
            self.logger.info(f"Connecting to {self.conf['connection']} to download {self.src}")
            # module_ = __import__(dropbox)
            classStr = self.conf['connection']+'Connector'
            res = getattr(dropbox, classStr)(self.src, self.dst, self.conf)
            return res.checkSrc()

    def move_to_history(self, files):
        """
        Moves the landing file to history zone
        """
        for file in files:
            src_landing = Path(self.dst/file)
            # print(f'src_landing: {src_landing}')
            dst_history = Path(str(src_landing).replace("landing","history"))
...
# class MultiTimerDatabaseHandler:
